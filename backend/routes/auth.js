  ...
  try {
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        return res
          .status(500)
          .json({ message: "Не вдалося надіслати код підтвердження" });
      } else {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!user.isEmailConfirmed) {
      return res
        .status(401)
        .json({ message: "Підтвердіть e‑mail, перш ніж увійти" });
    }
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(200)
        .json({ message: "Якщо email існує, лист надіслано" });
    }
...
router.post("/reset-password/:token", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json({ message: "Код недійсний або протермінований" });
    }
...
router.patch("/change-name", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (existingName)
      return res
        .status(409)
        .json({ message: "Такe ім`я вже використовується" });
